...
void List::Insert( ListItem *new_item )
{
    ...
    else    // Find first item larger than
            // inserted item
    {
       for (search=head; search;
            search=search->next)
          if (new_item->CompareByInsertKey(search)
              < 0)
             break;
             
       if (search==head) // Inserting new
                         // first item.
       {
          head = new_item;
       ...
       else if (! search) // Means insert as
                          // new last item.
       {
          tail->next = new_item;
          new_item->prev = tail;
...
void  List::Delete( ListItem *deleted_item )
{
    if (head == deleted_item)
       ...
       deleted_item->prev->next
          = deleted_item->next;

    if (tail == deleted_item)
       tail = deleted_item->prev;
    else
       deleted_item->next->prev 
          = deleted_item->prev;
